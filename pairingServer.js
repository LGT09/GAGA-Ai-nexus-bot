...
app.listen(PORT, () => {
  logger.info(`╔════════════════════════════════════════╗`)
  logger.info(`🌟 GAGA AI Pairing Server`)
  logger.info(`📍 Running on: http://localhost:${PORT}`)
  logger.info(`🤖 Bot Creator: ${CREATOR_INFO.name}`)
  logger.info(`©️  ${CREATOR_INFO.copyright}`)
  logger.info(`╚════════════════════════════════════════╝`)
  logger.info(`\n📱 Open http://localhost:${PORT} in your browser to start pairing\n`)
})
...
process.on('SIGINT', async () => {
  ...
  for (const [number, session] of activeSessions.entries()) {
    try {
      await session.socket.end()
      logger.info(`Disconnected: ${number}`)
    } catch (error) {
      logger.error(`Error disconnecting ${number}:`, error.message)
    }
  }
  process.exit(0)
})
